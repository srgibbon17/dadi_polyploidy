...
python3 path_to_script/dadi_demographic_inference.py \
    --config-file path_to_yaml_config/configs.yaml \
    --input-file path_to_tree_sequence_file/input.trees 

Explanation of command line arguments:
    config-file: Path to YAML configuration file specifying which demographic model to infer (required)
    input-file: Path to tree sequence file output from SLiM simulation (required)
        Note: in most cases, we will have, say, a set of 10 shorter tree sequences from SLiM 
        to maintain a reasonble compute time. In this case, the outputs will be labeled as 
        {name}_ID_1.trees, {name}_ID_2.trees, ..., {name}_ID_10.trees. 
        Importantly, the tree sequence file passed here should be the {name}_ID_1.trees file.

Examples of the config.yaml file for (1) an autotetraploid model with diploids and 
(2) an allotetraploid model are provided in the folder.
"""
